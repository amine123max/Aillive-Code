...
这个仓库用于发布 npm 包 `@aillive/cli`。安装后可使用两个命令：
...
Aillive Code 现在按 npm workspace 组织，同时由根包发布 `@aillive/cli` npm 包。可执行 CLI app 位于 `apps/cli`，根目录 `src/index.js` 保留为兼容 shim，方便旧测试和旧导入路径继续工作。
