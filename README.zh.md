...
产品结构参考了 MiMo Code、Codex、Claude Code、Droid 等成熟 AI Coding CLI 的开发者体验：快速安装、浏览器登录、slash commands、项目记忆、单次执行、本地配置和适合 CI 的 JSON 输出。Aillive Code 保留自己的命令命名、认证流程、API 协议和本地目录结构。
...
`npm run smoke:npx` 会创建临时 npm tarball、启动 mock Aillive API，并验证 `npm exec --package <tarball> -- aillive chat --json "Hello"` 可以通过 npm 执行路径调用 CLI。
